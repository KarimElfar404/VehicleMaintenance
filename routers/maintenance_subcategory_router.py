...
def get_all_maintenance_subcategory(db: Session = Depends(get_db)):
...
def get_maintenance_subcategory(subcategory_id: int, db: Session = Depends(get_db)):
...
def create_maintenance_subcategory(newSubcategory: MaintenanceSubcategoryCreate, db: Session = Depends(get_db)):
...
def update_maintenance_subcategory(subcategory_id: int, updateSubcategory: MaintenanceSubcategoryUpdate, db: Session = Depends(get_db)):
...
def delete_maintenance_subcategory(subcategory_id: int, db: Session = Depends(get_db)):
