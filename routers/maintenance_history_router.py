...
def create_manual_maintenance(vehicle_id:int, newMaintenance: ManualMaintenanceCreate, db: Session = Depends(get_db), current_user: User = Depends(require_permission("manual_maintenance:create"))):
...
def update_manual_maintenance(vehicle_id:int, maintenance_id: int, updateMaintenance: ManualMaintenanceUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_permission("manual_maintenance:update"))):
...
def delete_manual_maintenance(vehicle_id: int, maintenance_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_permission("manual_maintenance:delete"))):
