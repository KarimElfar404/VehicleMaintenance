...
def get_all_maintenance_category(db: Session = Depends(get_db)):
...
def get_maintenance_category(category_id: int, db: Session = Depends(get_db)):
...
def create_maintenance_category(newCategory: MaintenanceCategoryCreate, db: Session = Depends(get_db)):
...
def update_maintenance_category(category_id: int, updateCategory: MaintenanceCategoryUpdate, db: Session = Depends(get_db)):
...
def delete_maintenance_category(category_id: int, db:Session = Depends(get_db)):
